 ...
 * @author Notsawo
 */
public class PouleviewController implements Initializable {
    ...
    @FXML
    private TableView<VentePoule> factureTableView;
    @FXML
    private TableColumn<VentePoule, String> typePouleColumn;
    @FXML
    private TableColumn<VentePoule, Integer> quantitéColumn;
    @FXML
    private TableColumn<VentePoule, Integer> prixUnitaireColumn;
    @FXML
    private TableColumn<VentePoule, Integer> prixTotalColumn;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        factureTableView.setItems(VentePouleData);
        
        typePouleColumn.setCellValueFactory(cellData -> cellData.getValue().typeProperty());
...
// Les écouteurs
    ...
    private void handleAjouter() {
        if(!prixUnitaireSpinner.getEditor().getText().equalsIgnoreCase("0")){
            VentePouleData.add(new VentePoule(
                typeCombo.getValue(), 
    ...
    private void handleValiderFacture(ActionEvent event) {
        ...
        if (errorMessage.length() == 0) {
            ...
            System.out.println(VentePouleData);
            VentePouleData.forEach((vp) -> {
    ...
    private void handleModifier() {
        VentePoule vp  = factureTableView.getSelectionModel().getSelectedItem();
        
        VentePoule vpp  = new VentePoule(vp);
        ...
        if (vp != null) {
            if (fermeApp.showModifierFactureDialog(vp)){
                ...
                vp = new VentePoule(factureTableView.getItems().get(selectedIndex));
    ...
    */
    ...
    private void handleAjouterFacture(ActionEvent event) {
        ...
        VenteviewController.factureFinale.setVentePouleData(VentePouleData);
